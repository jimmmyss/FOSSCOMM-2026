<?php
/**
 * Sponsors — tiered, centered rows (Platinum / Gold / Silver / Community).
...
$by_tier = ['platinum' => [], 'gold' => [], 'silver' => [], 'community' => []];
...
// English-only base labels; the count is prepended into the eyebrow itself.
$tier_label = [
...
foreach (['platinum', 'gold', 'silver', 'community'] as $_t) {
...
?>
    <?php foreach ($by_tier as $tier => $items) :
        ...
        $label       = sprintf('%02d', $n) . ' / ' . $tier_label[$tier] . ($n === 1 ? '' : 's');
        ?>
...
if ($has_any_shine) :
    ...
    ?>
<style>
<?php foreach (['platinum', 'gold', 'silver', 'community'] as $_t) :
