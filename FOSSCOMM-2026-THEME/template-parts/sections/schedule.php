...
<section id="<?php echo esc_attr($id); ?>" class="bg-paper relative border-t border-border" <?php echo fc_island_attrs('schedule-filter'); ?>>
    ...
    >
        <select data-fc-filter="day" class="fc-sched-select" aria-label="Day">
            ...
            <?php foreach ($days as $dkey => $dmeta) :
                // Filter-dropdown label: "<English name> · <date>" (or just one
                // of them if the other is empty).
                $label = trim($dmeta['en'] !== '' ? $dmeta['en'] : $dmeta['el']);
                if ($dmeta['date'] !== '') {
        ...
        <select data-fc-filter="track" class="fc-sched-select" aria-label="Category">
            ...
            <?php foreach ($track_label as $slug => $name) : ?>
                <option value="<?php echo esc_attr($slug); ?>"><?php echo esc_html($name['en'] ?: $name['el']); ?></option>
            <?php endforeach; ?>
        </select>
    </nav>
    ...
    <div class="max-w-[1440px] mx-auto px-4 md:px-8 py-24 md:py-40">
        ...
        <?php endif; ?>
        <?php if ($title_en !== '') : ?>
            <h2 class="font-display text-4xl md:text-6xl leading-[1.0] tracking-tight mb-3" lang="en"><?php echo fc_format($title_en); ?></h2>
        <?php endif; ?>
        <?php if ($title_el !== '') : ?>
            <p class="font-display text-2xl md:text-3xl leading-tight tracking-tight text-ink-muted mb-16"><?php echo fc_format($title_el); ?></p>
        <?php elseif ($title_en !== '') : ?>
            <div class="mb-16"></div>
        <?php endif; ?>
            ...
            <div class="mb-16 last:mb-0" data-fc-day-list="<?php echo esc_attr($day_key); ?>" <?php echo $day_key === $default_day_key ? '' : 'hidden'; ?>>
                ...
                <div class="border-t-2 border-ink pt-4 mb-6">
                    <div class="flex items-baseline justify-between gap-4 font-mono text-[11px] uppercase tracking-widest text-ink-muted mb-2">
                        <span><?php echo esc_html(sprintf('Day %d', $day_index)); ?></span>
                        <span class="tabular-nums"><?php echo esc_html($day['date']); ?></span>
                    </div>
                    <div class="flex items-baseline gap-4 flex-wrap">
                        <?php if ($day['en'] !== '') : ?>
                            <h3 class="font-display text-4xl md:text-6xl leading-[0.95] tracking-tight m-0" lang="en"><?php echo esc_html($day['en']); ?></h3>
                        <?php endif; ?>
                        <?php if ($day['el'] !== '' && $day['el'] !== $day['en']) : ?>
                            <span class="font-display text-2xl md:text-3xl text-ink-muted leading-[1.0]"><?php echo esc_html($day['el']); ?></span>
                        <?php endif; ?>
                    </div>
                </div>
                        ...
                        <li class="fc-session border-b border-border"
                            ...
                            <div class="grid grid-cols-12 gap-4 py-6">
                                ...
                                <div class="col-span-9 md:col-span-7 min-w-0">
                                    <div class="font-display text-xl md:text-2xl leading-tight" lang="en"><?php echo fc_format($title['en']); ?></div>
                                    <?php if ($title['el'] !== '') : ?>
                                        <div class="font-display text-base md:text-lg text-ink-muted leading-tight mt-1"><?php echo fc_format($title['el']); ?></div>
                                    <?php endif; ?>
                                    <?php if ($speaker !== '') : ?>
                                    ...
                                    <?php if (!empty($row_tracks)) : ?>
                                        <div class="flex flex-wrap md:justify-end gap-1.5">
                                            <?php foreach ($row_tracks as $tslug) :
                                                ...
                                                $tname = $track_label[$tslug]['en'] ?: $track_label[$tslug]['el'];
                                                ?>
                            ...
                            <?php if ($prereq['el'] !== '' || $prereq['en'] !== '') : ?>
                                <div class="pb-6 pl-[calc(25%+1rem)] md:pl-[16.66%]">
                                    ...
                                    <?php if ($prereq['en'] !== '') : ?><p class="text-sm text-ink-muted leading-relaxed max-w-2xl" lang="en"><?php echo esc_html($prereq['en']); ?></p><?php endif; ?>
                                    <?php if ($prereq['el'] !== '') : ?><p class="text-sm text-ink-muted leading-relaxed max-w-2xl mt-1"><?php echo esc_html($prereq['el']); ?></p><?php endif; ?>
                                </div>
                            <?php endif; ?>
                        </li>
                    <?php endforeach; ?>
