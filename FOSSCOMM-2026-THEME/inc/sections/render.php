 ...
 * Eyebrow is English-only (the "section name").
 * Title renders English as the big heading, Greek as the smaller subtitle below.
 *
...
function fc_section_open(array $section, array $overrides = []): void {
    ...
    $eyebrow_en  = (string) ($overrides['eyebrow_en'] ?? ($section['eyebrow_en'] ?? ''));
    if ($eyebrow_en === '') {
        $eyebrow_en = (string) ($overrides['eyebrow_el'] ?? ($section['eyebrow_el'] ?? ''));
    }
    $title_el    = (string) ($overrides['title_el']   ?? '');
    $title_en    = (string) ($overrides['title_en']   ?? '');
