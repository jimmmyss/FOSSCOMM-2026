...
<section id="speakers" class="relative border-t border-border fc-section-dots">
    <div class="max-w-[1440px] mx-auto px-4 md:px-8 py-24 md:py-32">
        ...
        <?php else : ?>
            <?php /* Neither data-fc-speakers nor data-fc-centre. The first would
                     let the carousel claim this list — duplicating the cards and
                     setting them drifting. The second lit whichever card sat in
                     the middle of a touch screen, the way the belt does, and it
                     is gone on purpose: on the landing page the highlight tells
                     you which of the moving cards you are about to open, while
                     here nothing moves and it only made one card in a still grid
                     flicker orange as the page scrolled past. Every speaker
                     stays blue on a phone, and the hover below is for pointers
                     alone. */ ?>
            ...
            <ol class="fc-spk-grid"
                ...
                <?php foreach ($cards as $i => $card) : ?>
                    ...
                    get_template_part('template-parts/partials/speaker-card', null, [
                        ...
                        // No centre highlight here — see the note above the list.
                        'centre_item' => false,
                    ]);
                    ?>
                <?php endforeach; ?>
...
@media (hover: hover) {
    ...
    /* AND THE RING. This was the missing third declaration, and the reason a
       hovered speaker here lit its name and its photo while the outline round
       the cut-out stayed blue — on the landing row the whole highlight comes
       from `.fc-spk-card.is-hot` in the shared stylesheet, which switches the
       portrait between the two shared SVG filters; a grid card is hovered, not
       `.is-hot`, so that rule never fired and only the two above did. Same
       filter, same instant swap. */
    .fc-spk-grid .fc-spk-card:hover .fc-spk-photo {
        filter: url(#fc-spk-rim-hot);
    }
}
/* Nothing for `.is-hot` on a cell any more: this page no longer marks its cards
   for assets/centre-highlight.js, so on a touch screen every speaker holds the
   resting blue. */
</style>
