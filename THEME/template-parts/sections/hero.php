...
/* The open hand over a strip that can be dragged — but only where there is
   nothing to click: a strip WITH a link is the link, all of it, so the pointer
   from the site-wide rule in inc/bootstrap.php is the right cursor there and
   `:not([href])` steps out of its way (a plain class pair would otherwise
   outrank it on specificity). Once the hand is down the closed hand wins over
   either, link or not. Only while the strip is actually a carousel: a still one
   has nothing to drag. */
@media (hover: hover) {
    .fc-band.is-carousel:not([href]) { cursor: var(--fc-cur-grab, grab); }
...
@media (max-width: 1023.98px) {
    ...
    /* The links set like the Get Involved buttons on a phone: one per line,
       ranged left. 1rem apart — half of the 2rem this used to be: at that gap
       the three of them read as three separate things scattered down the panel
       rather than as one list of ways in, and on a short screen they pushed the
       email off the fold. */
    .fc-hero-actions ul {
        display: grid;
        justify-items: start;
