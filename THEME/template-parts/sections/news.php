...
fc_section_open($section, array_merge($meta, [
    'class'       => 'fc-section-dots',
    // The Manifesto's title exactly — the shared section-heading face and size
    // (assets/site.css). *Marked* runs still go through fc_format().
                ...
                <li class="fc-news bg-paper p-0 m-0 relative">
                    <a href="<?php echo esc_url($permalink); ?>"
                       class="flex flex-col gap-4 p-6 h-full no-underline text-inherit">
                        <div class="flex items-center justify-between fc-label text-ink-muted">
                        ...
                        <div class="aspect-video w-full border border-border bg-paper relative overflow-hidden">
                            ...
                            <?php else : ?>
                                <div class="absolute inset-0 flex items-center justify-center fc-label text-ink-faint">no image</div>
                            <?php endif; ?>
                        </div>
                        ...
                        <?php if ($body_text !== '') : ?>
                            <div class="fc-copy text-sm leading-relaxed text-ink-muted space-y-2">
                                <p class="m-0"><?php echo fc_format(wp_trim_words($body_text, 28, '…')); ?></p>
                            </div>
                        <?php endif; ?>

                        <span class="mt-auto fc-btn fc-btn-size text-ink-muted inline-flex items-baseline gap-2">
                            <span><?php echo esc_html(fc_t('read_more')); ?></span><span class="fc-arrow" aria-hidden="true">&gt;</span>
                        </span>
                    </a>
                </li>
            <?php endforeach; ?>
