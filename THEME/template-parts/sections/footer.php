...
$strip_arrow = static function (string $s): string {
        ...
        <?php foreach ($columns as $c) :
            ...
            <div>
                <?php if ($ctitle['el'] !== '' || $ctitle['en'] !== '') : ?>
                    <div class="fc-label text-ink-muted mb-3">
                        <?php echo fc_bi_inline($ctitle['el'], $ctitle['en']); ?>
                    </div>
                <?php endif; ?>

                <?php if ($has_body) : $cbody_text = fc_one($cbody); ?>
                    <div class="fc-copy text-sm leading-relaxed space-y-2 mb-4">
                ...
                <?php if (!empty($clinks)) : ?>
                    <ul class="text-sm leading-relaxed">
                        <?php foreach ($clinks as $link) :
                            ...
                            // Footer links carry no arrow at all. Drop one typed
                            // into the dashboard too, or it would still show.
                            $label = array_map($strip_arrow, fc_bi($link, 'label'));
                            $hover = array_map($strip_arrow, fc_bi($link, 'hover_label'));
                            $url   = (string) ($link['url'] ?? '');
