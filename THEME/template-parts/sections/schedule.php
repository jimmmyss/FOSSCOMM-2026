...
/* No section heading here any more — the day bands are the heading. Anything
   saved under fc_section_schedule is simply no longer read. */
...
<section id="<?php echo esc_attr($id); ?>" class="bg-paper relative border-t border-border" <?php echo fc_island_attrs('schedule-filter'); ?>>
    ...
    >
        <?php
        /* Each segment is a TEXT SPAN showing the current choice, with the real
         * <select> stretched invisibly over it.
         *
         * It used to be a bare <select> that fc.js sized to its text by measuring
         * a hidden copy — once, when the script ran. That is usually before the
         * webfont has loaded, so the width was measured in the narrower fallback
         * face and the real text was cut off ("SATURDAY · 17 OCT 202"). Nothing
         * is measured now: the span is as wide as its own text in whatever font
         * is actually on screen, reflows when the font arrives, and fc.js only
         * copies the chosen option's text into it on change. The select still
         * does all the work — clicks, keyboard, the native picker, the value. */
        $day_options = [];
        foreach ($days as $dkey => $dmeta) {
            // "<day name> · <date>" in the active language (or just one of them
            // if the other is empty).
            $label = trim(fc_pick($dmeta['el'], $dmeta['en']));
            if ($dmeta['date'] !== '') {
                $label = $label !== '' ? $label . ' · ' . $dmeta['date'] : $dmeta['date'];
            }
            if ($label !== '') $day_options[$dkey] = $label;
        }
        $day_shown = $day_options[$default_day_key] ?? fc_t('filter_day');
        ?>
        <?php if (!empty($days)) : $filter_count++; ?>
        <span class="fc-sched-seg">
            <span class="fc-sched-value" data-fc-filter-value aria-hidden="true"><?php echo esc_html($day_shown); ?></span>
            <select data-fc-filter="day" class="fc-sched-select" aria-label="Day">
                <option value=""><?php echo esc_html(fc_t('filter_day')); ?></option>
                <?php foreach ($day_options as $dkey => $label) : ?>
        ...
        <span class="fc-sched-seg">
            <span class="fc-sched-value" data-fc-filter-value aria-hidden="true"><?php echo esc_html(fc_t('filter_room')); ?></span>
        ...
        <?php if (!empty($track_label)) : ?>
            <?php if ($filter_count) : ?><span class="opacity-50">//</span><?php endif; $filter_count++; ?>
        <span class="fc-sched-seg">
            <span class="fc-sched-value" data-fc-filter-value aria-hidden="true"><?php echo esc_html(fc_t('filter_category')); ?></span>
    ...
    <div class="max-w-[1440px] mx-auto px-4 md:px-8 py-24 md:py-40">
        ...
        <?php endif; ?>
        <?php /* No heading. The day bands below carry the section — each one is
                 already a large display line with its own date — and a sentence
                 above them was a second heading for the same thing. The eyebrow
                 ("04 / Schedule") stays, and so does the sidebar entry. */ ?>
        ...
        <?php $day_index = 0; foreach ($by_day as $day_key => $rows) :
            ...
            <div class="mb-16 last:mb-0" data-fc-day-list="<?php echo esc_attr($day_key); ?>" <?php echo $day_key === $default_day_key ? '' : 'hidden'; ?>>
                ...
                <div class="border-t-2 border-ink pt-4 mb-6">
                    <div class="flex items-baseline justify-between gap-4 fc-label text-ink-muted mb-2">
                        ...
                        <li class="fc-session border-b border-border"
                            ...
                            <div class="grid grid-cols-12 gap-4 py-6">
                                ...
                                <div class="col-span-3 md:col-span-2 flex flex-col">
                                    ...
                                    <?php if ($room !== '') : ?>
                                        <div class="mt-3">
                                            <span class="inline-block fc-label text-ink-muted border border-border px-1.5 py-0.5 leading-none"><?php echo esc_html($room); ?></span>
                                        </div>
                                    <?php endif; ?>
                                </div>
                                <!-- Title + speaker. -->
                                    ...
                                    <?php if (!empty($row_tracks)) : ?>
                                        <div class="flex flex-wrap md:justify-end gap-1.5">
                                            <?php foreach ($row_tracks as $tslug) :
                                                ...
                                                ?>
                                                <span class="fc-label text-ink-muted border border-border px-1.5 py-0.5 leading-none"><?php echo esc_html($tname); ?></span>
                                            <?php endforeach; ?>
                                        </div>
                                    <?php endif; ?>
                                    <?php if ($lang !== '') : ?>
                                        <div class="fc-label text-ink-muted"><?php echo esc_html($lang); ?></div>
                                    <?php endif; ?>
                                </div>
                            </div>
                            <?php $prereq_text = fc_one($prereq); if ($prereq_text !== '') : ?>
                                <div class="pb-6 pl-[calc(25%+1rem)] md:pl-[16.66%]">
                                    <div class="fc-label text-accent mb-1"><?php echo esc_html(fc_t('before_workshop')); ?></div>
...
<style>
/* Filter segments: the visible text is .fc-sched-value, and the native <select>
   is laid over it, invisible, so a click anywhere on the text opens the OS/browser
   picker. The segment is exactly as wide as its text — no measuring, so nothing
   can be cut off. See the note above the markup. */
.fc-sched-seg {
    position: relative;
    display: inline-flex;
    align-items: center;
    ...
    /* Instant, like every other colour change in the chrome. */
    transition: none;
}
.fc-sched-value { white-space: nowrap; pointer-events: none; }
.fc-sched-seg:hover { color: var(--accent); }        /* blue on hover */
/* Keyboard focus lands on the invisible select, so the ring goes on the segment. */
.fc-sched-seg:has(.fc-sched-select:focus-visible) {
...
.fc-sched-select {
    ...
    margin: 0;
    padding: 0;
    /* 16px, not the bar's size: the select is invisible, so its size shows
       nowhere — and iOS zooms the page into any select under 16px on tap. */
    font-size: 16px;
