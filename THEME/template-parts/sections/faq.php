...
.fc-faq-line .fc-link.fc-link::after {
    content: ">";
    margin-left: 0.3em;
    /* The row must not grow the moment an answer with a link in it opens. Two
       things used to make it: the arrow was an inline-BLOCK, whose own line box
       counts as a tall atomic item in the row's, and it is set in Qaroxe, whose
       baseline sits at a different depth from the question's face (see
       .fc-accent in assets/site.css). Back to a plain inline box with no height
       and neither applies — the glyph still draws on the baseline, and the row
       is exactly as tall with the arrow as without it. */
    line-height: 0;
