...
$render_edition_item = function (array $ed, string $extra_class) {
    ...
    $url = (string) ($ed['url'] ?? '');
    $tag = $url !== '' ? 'a' : 'button';
    // No transition-colors: the editions switch colour instantly, like the rest
    // of the chrome (see .fc-num in assets/site.css).
    $attrs = 'class="fc-year-btn no-underline text-ink-muted ' . esc_attr($extra_class) . '"'
    ...
    echo '<' . $tag . ' ' . $attrs . '>';
    // The year is picked out in the accent, like the number in an eyebrow.
    echo '<span class="fc-edition-text"><span class="fc-num">' . esc_html((string) $yr) . '</span>'
       . esc_html(' / ' . ucfirst($ct)) . '</span>';
    echo '</' . $tag . '>';
};
...
<section id="<?php echo esc_attr($id); ?>" class="bg-paper relative border-t border-border">
    ...
    <div class="max-w-[1440px] mx-auto px-4 md:px-8 py-24 md:py-40">
        <?php if ($eyebrow !== '') : ?>
            <div class="fc-eyebrow fc-label text-ink-muted mb-6">
                <?php echo fc_eyebrow_html($eyebrow); ?>
            </div>
        <?php endif; ?>
        <?php /* No section heading. The venue NAME is the heading here — it is set
                 at heading size already, and a sentence above it was a second one
                 saying the same thing. The eyebrow ("05 / Venue") stays, so the
                 section is still labelled and the sidebar nav is unchanged.
                 $title is left read from the option so the stored text survives;
                 nothing prints it. */ ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-start md:items-stretch">
            ...
            <?php /* Centred in the row's height, so a short venue card sits level
                     with the middle of the globe instead of hanging from the top
                     with a field of empty space under it. On a phone the grid is
                     one column and the card is its own height, where this does
                     nothing. */ ?>
            <div class="flex flex-col justify-center">
                ...
                <?php if (!empty($uni_lines) || !empty($addr_lines)) : ?>
                    ...
                    <?php endif; ?>
                        ...
                        <?php if (!empty($addr_lines)) : ?>
                            ...
                            <div class="fc-venue-addr fc-label">
                ...
                <?php if (!empty($info_rows)) : ?>
                    <?php // No border-t: the rule that separated the address from these rows is gone. ?>
                    <dl class="fc-venue-info m-0 p-0">
                        <?php foreach ($info_rows as $row) :
                            ...
                            ?>
                            <?php /* Label, a rule across the gap, the site's ">" and the value.
                                     Both halves are the section-nav's type exactly — same size,
                                     weight, tracking and the same muted ink. */ ?>
                            <div class="fc-info-row">
                                <dt class="fc-label text-ink-muted m-0">
                                    <?php echo fc_bi_inline($rlabel['el'], $rlabel['en']); ?>
                                </dt>
                                <i class="fc-info-leader" aria-hidden="true"></i>
                                <span class="fc-arrow fc-info-arrow" aria-hidden="true">&gt;</span>
                                <dd class="fc-label text-ink-muted m-0">
        ...
        <?php if (!empty($travel_cards)) : ?>
            <div class="grid grid-cols-12 gap-8 border-t border-border pt-12 mt-0">
                ...
                <div class="col-span-12 md:col-span-3 fc-label text-ink-muted">
                    <div><?php echo esc_html($gh); ?></div>
                </div>
                <?php // fc-copy: these cards' paragraphs are where the site's body
                      // colour comes from — the class is here so they take it from
                      // the same rule everything else does. ?>
                <div class="fc-copy col-span-12 md:col-span-9 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 text-lg leading-relaxed">
                    <?php foreach ($travel_cards as $card) :
                        ...
                        ?>
                        <div>
                            <?php // The buttons' face AND the buttons' size. ?>
                            <div class="fc-btn fc-btn-size text-ink mb-2"><?php echo fc_format($ct); ?></div>
                            <?php if ($cb !== '') : ?>
                                <div class="mt-3">
...
/* OUTSIDE the hover query, because a phone has to have it. There is no pointer
   on touch, so assets/centre-highlight.js sets .is-hot on this link while it is
   near the middle of the screen. Putting a highlight rule inside
   @media (hover: hover) is the mistake that made the speakers' mobile highlight
   do nothing at all. */
.fc-venue-name-link.is-hot { --fc-venue-rim-now: var(--fc-venue-rim-hover, #EE8101); }

/* NO TRANSITION. This used to fade over 50ms, and that is what made the name
   feel heavy against the rest of the site: the stroke only began to move once
   the pointer had arrived, so the swap always lagged the hand by a frame or
   two. The speakers' ring is an SVG filter and cannot interpolate at all — it
   is simply the other colour on the first frame — and the manifesto's marked
   runs have been put on the same footing. This is the third of the three. */
...
.fc-venue-addr {
    ...
    /* The face itself is .fc-label, in the markup — the section-nav's type, the
       tier line's, the info rows'. It used to be spelled out here (12px at every
       width), which was the same thing everywhere except on a phone, where the
       shared face steps down to 11px and this did not. */
}
/* Same reason as the name's lines: the box is the line of type, not the column
   it sits in. */
/* Same reason as the name's lines: the box is the line of type, not the column
   it sits in. (It was briefly centred under the name; back to ranged left.) */
.fc-venue-addr span { width: fit-content; }
.fc-venue-info { margin-top: 2rem; }

/* ── Info rows: label ── rule ── > ── value ──────────────────────────────────
   The rule is a flex spacer that takes whatever room is left between the two,
   so it stretches and shrinks with the value's length. No per-row hairline any
   more: the rule crossing the gap already says "these two belong together",
   and a second horizontal line under it was one line too many. */
.fc-info-row {
    display: flex;
...
.fc-info-row dt,
.fc-info-row dd { margin: 0; }
.fc-info-row dd { flex: 0 0 auto; }

/* THE OPTICAL MIDDLE OF THE TYPE, measured from the baseline the row aligns on:
   these labels are uppercase mono, so their ink runs from the baseline to the
   cap height and the middle of it is half of that. Both the rule and the ">"
   hang off this one number, which is why they line up with each other and with
   the words on either side. */
.fc-info-row { --fc-info-mid: 0.36em; }

.fc-info-leader {
    flex: 1 1 auto;
    /* An empty flex item baselines on its own bottom edge, which IS the row's
       baseline, so the rule draws just above the baseline and is then lifted to
       the middle of the type.
       The lift is the middle MINUS half the rule's own thickness: the border
       hangs below the point it is positioned from, and without that half pixel
       the line rides visibly high of the ">" beside it. */
    border-bottom: 1px solid var(--color-border, color-mix(in oklab, #0A0A0A 12%, transparent));
    transform: translateY(calc(-1 * var(--fc-info-mid) + 0.07em));
}

/* The ">" is Qaroxe, whose ink sits high in its own em box. The size below is
   the one that looks right beside 12px type; the nudge then drops it onto the
   type's middle — MEASURED in the browser rather than derived, because the ink
   box the engine reports for this bitmap face at this size is a shade taller
   than the outlines say (its centre came out 5.0px above the baseline against
   the text's 4.0px, hence 1px, which is 0.083em of the row). */
.fc-info-arrow {
    flex: 0 0 auto;
    font-size: 1.29em;
    line-height: 1;
