...
<section class="bg-paper border-t border-b border-border">
    <div class="max-w-[1200px] mx-auto px-4 md:px-8 py-24 md:py-32">
        ...
        <?php /* The sections' heading, not a set of numbers of its own —
                 .fc-section-heading in assets/site.css, the one the Manifesto,
                 Sponsors and Speakers titles share. It carries its own gap
                 underneath, so nothing after it adds a top margin. */ ?>
        <h1 class="fc-section-heading">
            <?php echo fc_format($title); ?>
        </h1>

        <?php if ($intro !== '') : ?>
            <div class="fc-copy text-lg leading-relaxed max-w-3xl">
                            ...
                            <a href="<?php echo esc_url($url); ?>" target="_blank" rel="noreferrer"
                               class="group no-underline flex items-baseline gap-4 py-5 hover:text-accent transition-colors">
                                ...
                                <span class="flex-1">
                                    ...
                                    <?php if ($host !== '') : ?>
                                        <span class="fc-label text-ink-muted mt-1 block group-hover:text-accent transition-colors"><?php echo esc_html($host); ?></span>
                                    <?php endif; ?>
                                </span>
                                <span class="fc-arrow text-sm shrink-0" aria-hidden="true">&gt;</span>
                            </a>
                        <?php else : ?>
