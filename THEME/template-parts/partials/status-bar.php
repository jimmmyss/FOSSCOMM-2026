...
<header class="fixed top-0 inset-x-0 z-50 fc-bar border-b border-border bg-paper fc-label<?php echo fc_is_landing_page() ? ' fc-topbar-blue' : ''; ?>" <?php echo fc_island_attrs('status-bar', ['eventStart' => fc_get_event_start_iso()]); ?>>
    ...
    <div class="h-full px-4 lg:pl-5 lg:pr-8 flex items-center gap-4 text-ink-muted whitespace-nowrap overflow-x-auto fc-nav-no-scrollbar">
        ...
        <span class="opacity-50 shrink-0">//</span>
        <?php /* fc-topbar-item: the location and the countdown take the brand's
                 hover colour even though neither is a link — the bar reads as one
                 row of chrome, and half of it lighting up looked like a bug. */ ?>
        <span class="fc-topbar-item shrink-0"><?php echo fc_bi_inline($location_el, $location_en); ?></span>
        <span class="opacity-50 shrink-0">//</span>
        <span data-fc-countdown-clock class="fc-topbar-item shrink-0">…</span>
    </div>
</header>
