 ...
 * title_class — the heading's own classes, for a section that wants the big
 * display heading (.fc-section-heading) rather than the default one — and
 * title_href, which turns the heading into a link to a page that carries the
 * whole of what the section samples, with the site's ">" after it.
 */
function fc_section_open(array $section, array $overrides = []): void {
    ...
    $title_class = (string) ($overrides['title_class'] ?? 'font-display text-4xl md:text-6xl leading-[1.0] tracking-tight mb-16');
    ...
    <section id="<?php echo esc_attr($id); ?>" class="<?php echo esc_attr($bg_class); ?> relative border-t border-border <?php echo esc_attr($extra_class); ?>">
        <div class="max-w-[1440px] mx-auto px-4 md:px-8 py-24 md:py-40">
            <?php if ($eyebrow !== '') : ?>
                <?php // fc-eyebrow: hoverable like a nav link, with its number in the accent. ?>
                <div class="fc-eyebrow fc-label text-ink-muted mb-6">
                    <?php echo fc_eyebrow_html($eyebrow); ?>
                </div>
            <?php endif; ?>
            <?php if ($title !== '') : ?>
                <h2 class="<?php echo esc_attr($title_class); ?>"><?php
                    $title_href = (string) ($overrides['title_href'] ?? '');
                    if ($title_href !== '') :
                        /* The ">" is the site's own arrow — the same span the
                           buttons and the sponsor tiers use, so it is the same
                           face and goes orange on hover with them. aria-hidden,
                           because it is punctuation: the link already reads as
                           the heading. */
                        ?><a class="fc-section-heading-link" href="<?php echo esc_url($title_href); ?>"><?php
                            echo fc_format($title);
                            ?><span aria-hidden="true" class="fc-arrow">&gt;</span></a><?php
                    else :
                        echo fc_format($title);
                    endif;
                ?></h2>
