...
require_once FC_THEME_DIR . '/inc/speakers.php';
require_once FC_THEME_DIR . '/inc/seo.php';
...
function fc_enqueue_assets() {
    ...
    // Speakers row: decides at runtime whether the cards are wider than the
    // screen, and if they are, copies them and hands the stylesheet the two
    // numbers its animation needs. Nothing runs per frame — the belt is a CSS
    // animation. Inert when the section is absent: it looks for
    // [data-fc-speakers] and stops if there is none.
    wp_enqueue_script(
        'fc-speakers',
        FC_THEME_URI . '/assets/speakers-belt.js',
        [],
        FC_THEME_VERSION,
        true
    );
    // Hold and drag, for anything that drifts: the speakers row and the hero's
    // three strips. It only ever writes an offset on the element it is given —
    // the drift itself stays a CSS animation. Inert without [data-fc-drag].
    wp_enqueue_script(
        'fc-belt-drag',
        FC_THEME_URI . '/assets/belt-drag.js',
        [],
        FC_THEME_VERSION,
        true
    );
    // Sponsor tiers: the same measured decision as the speakers row, per tier —
    // a line of logos that fits stays a centred row, one that does not becomes a
    // drifting, draggable belt. Inert when the section is absent: it looks for
    // [data-fc-belt] and stops if there is none.
    wp_enqueue_script(
        'fc-logo-belt',
        FC_THEME_URI . '/assets/logo-belt.js',
        [],
        FC_THEME_VERSION,
        true
    );
    ...
    // Every link that leaves the site opens in a new tab — including the ones
    // typed into the dashboard, since it works on the anchors rather than on
    // whichever template printed them. Links back into this site are untouched.
    wp_enqueue_script(
        'fc-external-links',
        FC_THEME_URI . '/assets/external-links.js',
        [],
        FC_THEME_VERSION,
        true
    );
