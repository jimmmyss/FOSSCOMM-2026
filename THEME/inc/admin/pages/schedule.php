...
function fc_admin_page_schedule() {
    ...
    fc_render_collection_admin_page([
        ...
        'render_before' => function ($rows) use ($days, $day_fields, $tracks, $track_fields) {
            /* The "Section heading" field is gone with the heading it fed: the
               section leads with its day bands now. Nothing writes to
               fc_section_schedule any more, so an old heading stays in the
               database untouched should it ever be wanted back. */
            ?>
            <p class="description" style="margin:0 0 1.25rem;">
                This section has no heading of its own — the day bands below are the heading.
            </p>
            <?php
