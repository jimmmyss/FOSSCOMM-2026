...
function fc_admin_page_hero() {
    ...
    fc_render_section_admin_page([
        ...
        'render_form' => function ($values) use ($social_fields, $cta_fields) {
            ...
            <?php
            // A link for each strip — the whole strip becomes clickable when it is set.
            $strip_link = function ($key, $what) use ($values) {
                ?>
                <div class="fc-field">
                    <label><?php echo esc_html($what); ?> — link (optional)</label>
                    <input type="text" name="fc_field[<?php echo esc_attr($key); ?>]" value="<?php echo esc_attr((string) ($values[$key] ?? '')); ?>" placeholder="#venue or https://…">
                    <p class="description">Makes the whole strip a link. A full address (<code>https://…</code>) or an on-page anchor such as <code>#venue</code> or <code>#schedule</code>. Leave empty for no link.</p>
                </div>
                <?php
            };
