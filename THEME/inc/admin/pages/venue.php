...
function fc_admin_page_venue() {
    ...
    fc_render_section_admin_page([
        ...
        'render_form' => function ($values) use ($card_fields, $edition_fields, $info_fields) {
            /* The "Section title" field is gone with the heading it fed: the
             * section leads with the venue NAME now, which is already set at
             * heading size, so a sentence above it said the same thing twice.
             *
             * The two hidden inputs keep whatever was typed there. `title` is
             * still in the schema, and this form REPLACES the whole option on
             * save — without them, the first save after this change would quietly
             * blank the stored text, and putting the heading back would mean
             * writing it again. */
            ?>
            <p class="description" style="margin:0 0 1.25rem;">
                This section has no heading of its own — the venue name below is the heading.
                Any section title you had saved is kept, just not shown.
            </p>
            <input type="hidden" name="fc_field[title_el]" value="<?php echo esc_attr((string) ($values['title_el'] ?? '')); ?>">
            <input type="hidden" name="fc_field[title_en]" value="<?php echo esc_attr((string) ($values['title_en'] ?? '')); ?>">
            <?php
