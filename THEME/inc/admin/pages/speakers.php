...
function fc_admin_page_speakers() {
    ...
    $fields = [
        ...
        'featured' => [
            'type'    => 'bool',
            'label'   => 'Highlight',
            // Ticked for a new speaker, and for every speaker saved before this
            // field existed (see the 'bool' case in fc_repeater_field_input and
            // fc_speakers_featured): adding the field must not empty the row on
            // the landing page.
            'default' => true,
            'help'    => 'Show in the row on the landing page (/speakers/ lists everyone either way)',
        ],
        'online' => [
            'type'  => 'bool',
    ...
    fc_render_collection_admin_page([
        ...
        'intro'      => 'One entry per speaker: the <strong>name</strong> large, the <strong>roles</strong> '
                      ...
                      . 'Two lists, one set of speakers. <strong>Highlight</strong> decides who appears in '
                      . 'the row on the landing page; the page at <code>/speakers/</code> — linked from that '
                      . 'row\'s heading — lists <strong>everyone</strong>, ticked or not. Untick a speaker to '
                      . 'take them off the landing page without removing them from the site.<br><br>'
        ...
        'render_before' => function ($rows) use ($defaults) {
            ...
            ?>
            <h2 style="margin-top:2rem;">Heading on the speakers page</h2>
            <p class="description">The heading at the top of <code>/speakers/</code>, where every speaker is
               listed — the section heading above belongs to the row on the landing page, which shows only
               the ticked ones. Leave this blank to use that same heading.</p>
            <?php
            $page_meta = get_option('fc_section_speakers', []);
            if (!is_array($page_meta)) $page_meta = [];
            fc_bilingual_field('page_title', $page_meta, [
                'type'        => 'text',
        ...
        'post_process' => function ($clean, $raw) {
            // The page heading rides in the same meta option as the section
            // heading, so it has to be declared here or the save would drop it.
            fc_section_meta_save('speakers', $raw, ['page_title' => 'bilingual']);
