...
function fc_admin_sections_page() {
    ...
    if (
        ...
        foreach (fc_section_registry() as $key => $def) {
            ...
            // Bilingual name overrides (label + eyebrow). Only store rows that
            // actually carry an override; empty fields fall back to the registry.
            $row = [];
            foreach (['label_el', 'label_en', 'eyebrow_el', 'eyebrow_en'] as $f) {
                $row[$f] = sanitize_text_field((string) wp_unslash($entry[$f] ?? ''));
            }
        ...
        update_option('fc_section_names', $names, false);
    ...
    $names    = get_option('fc_section_names', []);
    ...
    <div class="wrap fc-wrap">
        ...
        <div class="fc-callout">
            <strong>Drag rows to reorder.</strong> Untick "Active" to hide a section from the landing page and the nav. Edit the <strong>nav name</strong> (top input) and <strong>eyebrow</strong> (lower input, e.g. "01 / Manifesto") per language — leave blank to use the built-in default. Click <em>Edit</em> on any section with an admin page to edit its copy.
        </div>
        <form method="post">
            ...
            <table class="fc-sections-table">
                <thead>
                    <tr>
                        ...
                        <th><?php esc_html_e('Name / eyebrow (EL)', 'fosscomm'); ?></th>
                        <th><?php esc_html_e('Name / eyebrow (EN)', 'fosscomm'); ?></th>
                ...
                <?php foreach ($ordered as $key => $def) :
                    ...
                    $ovr       = is_array($names[$key] ?? null) ? $names[$key] : [];
                    $name_input = function (string $field, string $placeholder, string $value, bool $small) use ($key) {
                        printf(
                            '<input type="text" name="fc_section[%s][%s]" value="%s" placeholder="%s" style="width:100%%;%s">',
                            esc_attr($key), esc_attr($field), esc_attr($value), esc_attr($placeholder),
                            $small ? 'margin-top:4px;font-size:11px;font-family:ui-monospace,Menlo,monospace;' : ''
                        );
                    };
                    ?>
                    <tr class="<?php echo $active ? '' : 'inactive'; ?>">
                        ...
                        <td>
                            <?php
                            $name_input('label_el',   (string) $def['label_el'],   (string) ($ovr['label_el'] ?? ''),   false);
                            $name_input('eyebrow_el', (string) $def['eyebrow_el'], (string) ($ovr['eyebrow_el'] ?? ''), true);
                            ?>
                        </td>
                        <td>
                            <?php
                            $name_input('label_en',   (string) $def['label_en'],   (string) ($ovr['label_en'] ?? ''),   false);
                            $name_input('eyebrow_en', (string) $def['eyebrow_en'], (string) ($ovr['eyebrow_en'] ?? ''), true);
                            ?>
                        </td>
                        <td>
                            <?php if ($edit_url) : ?>
 ...
 *
 * $extra_schema adds fields that a page renders into the same `fc_section_meta`
 * name prefix — the Speakers page's second heading, for /speakers/. It has to be
 * declared: the write REPLACES the option, so anything not in the schema is
 * dropped on the next save.
 */
function fc_section_meta_save(string $section_key, array $post, array $extra_schema = []): void {
    $raw   = isset($post['fc_section_meta']) && is_array($post['fc_section_meta']) ? $post['fc_section_meta'] : [];
    $clean = fc_sanitize_fields($raw, array_merge(['title' => 'bilingual'], $extra_schema));
