...
function fc_seed_initial_content() {
    ...
    // NO SEEDED SPONSORS, deliberately — `fc_sponsors` starts empty.
    //
    // It used to ship two dozen invented tiers and sponsor names ("Crete Cloud",
    // "Polytechnic Press"), which is worse than it sounds for this particular
    // section: a made-up sponsor is a claim about who is funding the conference.
    // The other seeded sections are copy somebody will rewrite; this one is a
    // list somebody has to REMEMBER to empty, and until they do the site is
    // publishing names that never agreed to be there.
    //
    // Empty means fc_sponsor_tiers() returns nothing and the section renders its
    // "to be announced" state, which is the honest thing to show before any
    // sponsor has signed. Nothing else needs to change: the migration recognises
    // an empty option and leaves it alone.
    ...
    /* The money moved to the sponsors section, where it is the headline:
       "<words> €<amount> <words>", with the goal deciding how full the number is
       drawn. Seeded with the sentence but NO figures — an invented amount raised
       is a claim about real money, so it stays at zero until someone types one. */
    fc_seed_if_empty('fc_sponsors_funding', [
        'part1_el' => 'Μέχρι στιγμής μαζέψατε',
        'part1_en' => 'So far you have raised',
        'part2_el' => 'από το «δωρεάν».',
        'part2_en' => 'of ‘free’.',
        'amount'   => 0,
        'goal'     => 0,
    ]);
    ...
    fc_migrate_sponsors_to_tiers();
}

/**
 * One-time migration: fold the flat sponsor list and its two side-options into
 * one nested list of tiers.
 *
 * Before: `fc_sponsors` was a flat list of sponsors, each naming one of six
 * hard-coded slugs; `fc_sponsors_tiers` held those slugs' bilingual labels and
 * `fc_sponsors_shine` their colours. Three options, one hard-coded vocabulary,
 * and no way to add a seventh tier without editing four files.
 *
 * After: `fc_sponsors` is the tiers, each holding its own sponsors.
 *
 * What carries over, and what deliberately does not:
 *   • ORDER. Tiers come out in the order they were displayed in, and each tier's
 *     sponsors keep the order they were entered in — the page looks the same
 *     afterwards, which is the whole test of a migration like this.
 *   • Only tiers that HAVE sponsors become tiers. An empty Bronze that nobody
 *     ever used should not arrive as an empty heading to delete by hand.
 *   • The label becomes the TITLE with no asterisks added. Which part is hollow
 *     is an authoring decision and guessing it wrong is worse than leaving the
 *     heading solid until somebody marks it.
 *   • `desc` is left empty. It is a new field; there is nothing to carry.
 *   • A tier that had no shine colour still gets one (the house blue), because
 *     the colour now also draws the title and a tier with no colour would have
 *     an invisible heading. `shine` records whether the sweep was actually on.
 *
 * The three old options are NOT deleted. They cost nothing left in place, and if
 * this ever turns out to have folded something wrongly they are the only copy of
 * what it was folded from.
 */
function fc_migrate_sponsors_to_tiers(): void {
    if (get_option('fc_sponsors_tiers_v1') === '1') return;
    update_option('fc_sponsors_tiers_v1', '1', true);

    $rows = get_option('fc_sponsors', []);
    if (!is_array($rows) || !$rows) return;
    // Already nested — a fresh install seeded in the new shape, or this ran on
    // another request first.
    foreach ($rows as $row) {
        if (is_array($row) && array_key_exists('sponsors', $row)) return;
    }

    $shine = get_option('fc_sponsors_shine', []);
    if (!is_array($shine)) $shine = [];
    $legacy_colour = trim((string) get_option('fc_sponsors_shine_color', ''));

    // The display order the template used, so the page does not reshuffle.
    $order = ['diamond', 'gold', 'silver', 'bronze', 'community', 'in-kind'];

    $by_tier = array_fill_keys($order, []);
    foreach ($rows as $sp) {
        if (!is_array($sp)) continue;
        $tier = strtolower(trim((string) ($sp['tier'] ?? 'community')));
        if ($tier === 'platinum') $tier = 'diamond';        // the old rename
    ...
    $tier_opt = get_option('fc_sponsors_tiers', []);
    if (!is_array($tier_opt)) $tier_opt = [];
    $defaults = fc_sponsor_tier_defaults();

    $out = [];
    foreach ($order as $tier) {
        if (!$by_tier[$tier]) continue;

        $saved = is_array($tier_opt[$tier] ?? null) ? $tier_opt[$tier] : [];
        if (!$saved && $tier === 'diamond' && is_array($tier_opt['platinum'] ?? null)) {
            $saved = $tier_opt['platinum'];
        }
        $el = trim((string) ($saved['label_el'] ?? ''));
        $en = trim((string) ($saved['label_en'] ?? ''));
        if ($el === '') $el = (string) ($defaults[$tier]['el'] ?? '');
        if ($en === '') $en = (string) ($defaults[$tier]['en'] ?? '');

        $colour = trim((string) ($shine[$tier] ?? ''));
        if ($colour === '' && $tier === 'diamond') {
            $colour = trim((string) ($shine['platinum'] ?? ''));
        }
        if ($colour === '') $colour = $legacy_colour;
        $had_shine = $colour !== '';
        // sanitize_hex_color() rejects `gold` and `rgba(…)`, both of which the old
        // free-text field allowed. Those cannot carry into a picker, so they fall
        // back to the house blue rather than saving a value the field will not
        // show — the tier keeps its shine, just in a colour worth re-picking.
        $hex = $colour === '' ? '' : (string) (sanitize_hex_color($colour) ?? '');
        if ($hex === '') $hex = FC_OUTLINE_REST;
        ...
        $out[] = [
            ...
            'colour'   => $hex,
            'shine'    => $had_shine,
            'sponsors' => $by_tier[$tier],
        ];
    }

    update_option('fc_sponsors', $out, false);
}
...
function fc_migrate_volunteer_cards(): void {
    ...
    // Saved in the three-column shape (fc_involved_values()): there are no cards
    // any more, and "no cards" must not read as "untouched old defaults".
    if (array_key_exists('p_title1_en', $vol)) return;
