...
function init() {
  ...
  // Only the sections the nav actually lists. The footer is a section with an id
  // like any other, so scrolling into it used to make it "the active section" —
  // and since no link points at it, setActive() then turned every link off and
  // the sidebar went blank at the bottom of the page. Skipping the ones with no
  // link leaves the last listed section (the FAQ) lit while you read the footer,
  // which is where you still are as far as the nav is concerned.
  const navKeys = new Set(links.map((link) => link.dataset.fcNavTarget).filter(Boolean))

  function measureTops() {
    const scrollY = window.scrollY
    tops = []
    document.querySelectorAll('section').forEach((sec) => {
      if (sec.id && navKeys.has(sec.id)) {
        tops.push({ id: sec.id, top: sec.getBoundingClientRect().top + scrollY })
      }
    })
  ...
  function measureSectionsEnd() {
    ...
    if (!navEl) return
    // Bar height comes from the shared --fc-bar-h token (assets/site.css), the
    // same value every sticky bar locks to.
    const barH = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--fc-bar-h')) || 40
    document.documentElement.style.setProperty('--fc-sections-end', (barH + navEl.offsetHeight) + 'px')
  }
