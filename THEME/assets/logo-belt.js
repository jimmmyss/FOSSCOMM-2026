...
(function () {
    ...
    /* ── Is the pointer on the MARK, or merely in its box? ───────────────────
     *
     * A logo arrives as a rectangle with a drawing somewhere inside it, and for
     * a wordmark most of that rectangle is transparent paper. Treating the box
     * as the link means the hand appears — and a click lands — over the empty
     * strip between two marks, which reads as the row being one big button.
     *
     * So the artwork's alpha channel is read ONCE per image into a 64x64 map and
     * kept on the element; every test after that is one array lookup, done at
     * most once a frame. This is the speakers row's test (assets/speakers-belt.js)
     * with the mapping changed: a portrait stands on the floor of its box, a logo
     * is centred in one.
     */
    var GRID = 64;          // the alpha map, per side
    var OPAQUE = 40;        // 0-255; below this the pixel is paper, not mark
    ...
    function alphaMap(img) {
        ...
        try {
            ...
            /* A logo served from another origin taints the canvas and cannot be
               read. Rather than make that sponsor unclickable, the whole box
               counts as the mark — which is exactly how the row behaved before
               any of this. */
            map = null;
        }
        img.__fcAlpha = map;
    ...
    function onInk(img, x, y) {
        ...
        if (!img.complete || !img.naturalWidth) return true;   // not decoded yet
        var map = alphaMap(img);
        if (!map) return true;

        // `object-fit: contain`, centred (the CSS sets no object-position). The
        // box is built to the artwork's own ratio, so the letterbox is usually
        // nothing — but an unknown or capped shape does letterbox, and then the
        // bands either side are not the picture.
        var scale = Math.min(r.width / img.naturalWidth, r.height / img.naturalHeight);
        var dw = img.naturalWidth * scale;
        var dh = img.naturalHeight * scale;
        var u = (x - (r.left + (r.width - dw) / 2)) / dw;
        var v = (y - (r.top + (r.height - dh) / 2)) / dh;
    ...
    /** The sponsor link under (x, y), or null if the pointer is on paper. */
    function logoAt(target, x, y) {
        if (!target || !target.closest) return null;
        var cell = target.closest('a.fc-belt-cell');
        if (!cell) return null;
        // A sponsor with no logo file is drawn as its NAME. Words are the mark
        // there, and the browser's own hit test on the text is the right answer.
        if (target.closest('.fc-belt-name')) return cell;
        var img = cell.querySelector('.fc-belt-logo:not(.fc-belt-logo-alt)');
        if (img && onInk(img, x, y)) return cell;
        return null;
    }
    ...
    function init(wrap) {
        ...
        // Whether the pointer is currently over a mark, and the move waiting to
        // be tested on the next frame.
        var onLogo = false;
        var pendingHover = null;
        var hoverFrame = 0;

        function setOnLogo(on) {
            if (on === onLogo) return;
            onLogo = on;
            wrap.classList.toggle('is-on-logo', on);
        }
        ...
        function hoverPaint() {
            ...
            pendingHover = null;
            if (!e) return;
            setOnLogo(!!logoAt(e.target, e.clientX, e.clientY));
        }
        ...
        /* Two clicks are swallowed here, both in the capture phase so the <a>
           never sees them. Otherwise a sponsor cell is a plain link and the
           browser's own click is correct.
             • the one a DRAG produced — a gesture, not a press;
             • one on the transparent paper around a mark, which looked like a
               press on the row rather than on that sponsor.
           A keyboard activation carries no coordinates (detail 0) and is left
           alone: tabbing to a logo and pressing Enter must always work. */
        viewport.addEventListener('click', function (e) {
            var stop = false;
            if (suppressClick) {
                suppressClick = false;
                stop = true;
            } else if (e.detail !== 0 && !logoAt(e.target, e.clientX, e.clientY)) {
                stop = true;
            }
            if (!stop) return;
            e.preventDefault();
        ...
        if (canHover) {
            ...
            /* The cursor, from the same test the click uses: the hand only over
               a mark. Coalesced to one test per frame — a pointer move can fire
               far more often than that, and each test reads the image's box. */
            viewport.addEventListener('pointermove', function (e) {
                pendingHover = e;
                if (!hoverFrame) hoverFrame = requestAnimationFrame(hoverPaint);
            }, { passive: true });
        }
