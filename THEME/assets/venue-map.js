    ...
    function initMap(mount) {
        ...
        function sassAtYear(year) {
            document.querySelectorAll('[data-fc-edition-year="' + year + '"]').forEach(function (b) {
                ...
                if (!txt) return;
                /* Keep the label's MARKUP, not just its text. The year is wrapped
                 * in <span class="fc-num"> so it can be painted in the accent;
                 * stashing textContent and putting it back dropped that span, and
                 * the year stayed grey for the rest of the visit — every row that
                 * had ever been clicked went dull.
                 *
                 * The stash lives on the element rather than in a data-attribute:
                 * it is markup, and markup does not belong in an attribute. */
                if (b._sassHTML == null) b._sassHTML = txt.innerHTML;
                var msg = nextSass(txt.getAttribute('data-fc-sass') || '');
                txt.setAttribute('data-fc-sass', msg);
                txt.textContent = msg;              // the message itself is plain text
                clearTimeout(b._sassT);
                b._sassT = setTimeout(function () { txt.innerHTML = b._sassHTML; }, 1400);
            });
        }
