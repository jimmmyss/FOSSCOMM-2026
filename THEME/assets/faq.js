    ...
    function bindItem(item) {
        ...
        function render(toAttr, open) {
            ...
            lines.forEach(function (line) {
                ...
                /* Either side may carry markup — a link, or a *marked* run in
                   the accent's pixel face — and it is animated THROUGH that
                   markup (fcScrambleHTML), not typed in flat and restyled at
                   the end. That swap is what made a highlighted phrase decode
                   in the body face and only turn blue once the whole line had
                   settled; now each character arrives already inside its own
                   span. The plain scramble stays as the path for a line with no
                   markup in it, and as the fallback if that script is missing. */
                var htmlAttr = line.getAttribute(open ? 'data-fc-a-html' : 'data-fc-q-html');
                var hasHtml  = (htmlAttr != null && htmlAttr !== '');
                if (hasHtml && typeof window.fcScrambleHTML === 'function') {
                    window.fcScrambleHTML(line, htmlAttr);
                } else if (typeof window.fcScramble === 'function') {
