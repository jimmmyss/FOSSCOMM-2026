 ...
 *   • The hover-text reveal is a desktop-pointer affordance only. A tap on a
 *     real link navigates immediately (no reveal-first / two-tap); blank links
 *     do nothing (the global dead-href guard swallows the tap).
 *
...
(function () {
    ...
    function bind(link) {
        ...
        // ----- Mobile -----
        // No tap-to-scramble: a tap on a real link navigates immediately (the
        // browser default) instead of first revealing the hover text and needing
        // a second tap. The hover-text reveal stays a desktop-pointer affordance.
        // Dead anchors are still swallowed by the global guard above, so a blank
        // CTA does nothing on tap (rather than scrambling to nowhere).
    }
