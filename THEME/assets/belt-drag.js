...
(function () {
    ...
    targets.forEach(function (el) {
        ...
        el.addEventListener('pointerdown', function (e) {
            ...
            /* NOT captured here — that is what broke clicking a speaker.
             *
             * A browser fires `click` at the nearest common ancestor of where the
             * press went down and where it came up. Capture retargets BOTH of
             * those to the capturing element, so with the capture taken on
             * pointerdown every click landed on this wrapper and the <a> inside
             * it was never activated: the link opened from the context menu,
             * which does not go through click, and from a touch, which browsers
             * still deliver to the element under the finger — exactly the split
             * that was reported. The capture is taken in pointermove instead, the
             * moment the press passes SLOP and becomes a drag; a press that never
             * travels that far is a click and is left entirely alone. */
        });
        ...
        el.addEventListener('pointermove', function (e) {
            ...
            /* Past SLOP this is a drag, not a click, so take the pointer: the
               moves keep coming when the hand runs off the edge of the row, and
               the click this release swallows below is one the link would not
               have wanted anyway. It throws if the pointer is not really down;
               the drag still works, just only over the element. */
            if (!captured && moved > SLOP) {
                captured = true;
        ...
        /* And on the window as well, for the press that never became a drag and
           so was never captured: let go a pixel outside the row and the element
           hears nothing, which would leave the belt paused and the hand closed
           for good. release() ignores a second call for the same pointer. */
